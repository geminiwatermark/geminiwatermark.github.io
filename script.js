(function () {
  ...
  /* Reference watermarks inlined as data URIs (always same-origin) */
  ...
  function calculateAlphaMap(imageData) {
    ...
    for (let i = 0; i < map.length; i++) {
      const idx = i * 4;
      map[i] = Math.max(data[idx], data[idx + 1], data[idx + 2]) / 255.0;
    }
    return map;
  }
  ...
  function calculateWatermarkPosition(w, h, cfg) {
    return {
  ...
  function removeWatermark(imageData, alphaMap, pos) {
    for (let row = 0; row < pos.height; row++) {
      for (let col = 0; col < pos.width; col++) {
        const imgIdx = ((pos.y + row) * imageData.width + (pos.x + col)) * 4;
        let a = alphaMap[row * pos.width + col];
        if (a < ALPHA_THRESHOLD) continue;
        a = Math.min(a, MAX_ALPHA);
        const inv = 1 / (1 - a);
        for (let c = 0; c < 3; c++) {
          const wm = imageData.data[imgIdx + c];
          const orig = (wm - a * LOGO_VALUE) * inv;
  ...
  function removeWatermarkFromImage(img) {
    const c = document.createElement('canvas');
    ...
    const cfg = detectWatermarkConfig(c.width, c.height);
    const pos = calculateWatermarkPosition(c.width, c.height, cfg);
    const map = alphaMaps[cfg.logoSize];
    if (!map) throw new Error('Engine not ready');
    removeWatermark(data, map, pos);
  ...
  async function bootEngine() {
    ...
    alphaMaps[48] = buildAlphaMapFromImage(bg48, 48);
    alphaMaps[96] = buildAlphaMapFromImage(bg96, 96);
  }

  bootEngine().catch(function(){});
  ...
  formatSeg.addEventListener('click', (e) => {
    ...
    if (!btn) return;
    $$('.seg-btn', formatSeg).forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    outputFormat = btn.dataset.format;
    if (currentFile) processFile(currentFile);
  });

  $$('.chip[data-format]').forEach(chip => {
    chip.addEventListener('click', () => {
      const fmt = chip.dataset.format;
      queue.forEach(item => { if (item.status === 'done') item.format = fmt; });
      renderBatch();
    });
  });
  ...
  downloadBtn.addEventListener('click', () => {
    ...
    const name = currentFile.name.replace(/\.[^.]+$/, '') + `-clean.${ext}`;
    saveBlob(processedBlob, name);
  });
  downloadAllBtn.addEventListener('click', () => {
    queue.forEach((item, i) => {
      if (item.status === 'done') {
        const ext = item.format.split('/')[1].replace('jpeg', 'jpg');
        saveBlob(item.blob, `image-${i + 1}-clean.${ext}`);
      }
    });
  });
  ...
  function handleFiles(files) {
    const valid = files.filter(f =>
      /^image\/(png|jpeg|webp)$/.test(f.type) && f.size <= 20 * 1024 * 1024
    );
    if (!valid.length) {
      setStatus('No supported files (JPG, PNG, WebP up to 20MB).');
      return;
    }
    if (!alphaMaps[48] || !alphaMaps[96]) {
      setStatus('Engine still loading, please wait…');
      return;
    }
    if (valid.length === 1) {
      queue.length = 0;
      ...
      currentFile = null;
      queue.length = 0;
      valid.forEach(f => queue.push({
        file: f, status: 'pending', blob: null, format: 'image/png', url: URL.createObjectURL(f)
      }));
      renderBatch();
      processQueue();
    }
  ...
  function renderBatch() {
    ...
    queue.forEach((item) => {
      ...
      status.textContent =
        item.status === 'done' ? 'Cleaned' :
        item.status === 'processing' ? 'Processing…' :
        item.status === 'error' ? 'Failed' : 'Queued';
      li.appendChild(thumb); li.appendChild(name); li.appendChild(status);
      batchList.appendChild(li);
    });
    const done = queue.filter(q => q.status === 'done').length;
    progressText.textContent = `${done}/${queue.length}`;
  }
  ...
  async function processQueue() {
    for (const item of queue) {
      ...
      try {
        ...
        const canvas = removeWatermarkFromImage(img);
        const blob = await canvasToBlob(canvas, item.format, 0.95);
        item.blob = blob;
        item.status = 'done';
      } catch (err) {
        item.status = 'error';
  ...
  async function processFile(file) {
    ...
    try {
      ...
      processedBlob = await canvasToBlob(canvas, outputFormat, 0.95);
      const elapsed = performance.now() - t0;
      const wait = Math.max(0, PROCESS_DELAY_MS - elapsed);
      if (wait > 0) await new Promise(r => setTimeout(r, wait));
      resultLoader.hidden = true;
      setStatus('Done. Click Download to save.');
    } catch (err) {
      resultLoader.hidden = true;
      setStatus('Failed to process: ' + (err.message || err));
    }
  }
